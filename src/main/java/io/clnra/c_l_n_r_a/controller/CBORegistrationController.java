...
public class CBORegistrationController {
    ...
    public void prepareContext(final Model model) {
        model.addAttribute("managementsValues", cBOManagementRepository.findAll().stream().collect(
                Collectors.toMap(CBOManagement::getId, CBOManagement::getFullName)));
        model.addAttribute("cBOPartnerssValues", partnersRepository.findAll().stream().collect(
                Collectors.toMap(Partners::getId, Partners::getPartnerName)));
        model.addAttribute("propertiessValues", cBOPropertiesRepository.findAll().stream().collect(
                Collectors.toMap(CBOProperties::getId, CBOProperties::getPropertyName)));
        model.addAttribute("projectsValues", cBOProjectsRepository.findAll().stream().collect(
                Collectors.toMap(CBOProjects::getId, CBOProjects::getProjectName)));
        model.addAttribute("activitiessValues", cBOActivitiesRepository.findAll().stream().collect(
                Collectors.toMap(CBOActivities::getId, CBOActivities::getActivityName)));
        model.addAttribute("cBOHuntingsValues", huntingRepository.findAll().stream().collect(
                Collectors.toMap(Hunting::getId, Hunting::getAnimalName)));
        model.addAttribute("wildlifePPsValues", gameCountRepository.findAll().stream().collect(
                Collectors.toMap(GameCount::getId, GameCount::getGameCount)));
        model.addAttribute("cBOTourismsValues", tourismRepository.findAll().stream().collect(
                Collectors.toMap(Tourism::getId, Tourism::getTourismLog)));
        model.addAttribute("humanConflictsValues", hWCRepository.findAll().stream().collect(
                Collectors.toMap(HWC::getId, HWC::getHWCLog)));
        model.addAttribute("aPsValues", antiPoachingRepository.findAll().stream().collect(
                Collectors.toMap(AntiPoaching::getId, AntiPoaching::getSiteName)));
        model.addAttribute("cSRCBOsValues", cSRRepository.findAll().stream().collect(
                Collectors.toMap(CSR::getId, CSR::getCRSLog)));
        model.addAttribute("cBOPatrolssValues", patrolsRepository.findAll().stream().collect(
                Collectors.toMap(Patrols::getId, Patrols::getPatrolsLog)));
    }
    ...
    public String add(
        ...
        if (!bindingResult.hasFieldErrors("cBOID") && cBORegistrationService.cBOIDExists(cBORegistrationDTO.getCBOID())) {
            bindingResult.rejectValue("cBOID", "Exists.cBORegistration.cBOID");
        }
    ...
    public String edit(@PathVariable final Long id,
            @ModelAttribute("cBORegistration") @Valid final CBORegistrationDTO cBORegistrationDTO,
            final BindingResult bindingResult, final RedirectAttributes redirectAttributes) {
        if (!bindingResult.hasFieldErrors("cBOID") &&
                !cBORegistrationService.get(id).getCBOID().equals(cBORegistrationDTO.getCBOID()) &&
                cBORegistrationService.cBOIDExists(cBORegistrationDTO.getCBOID())) {
            bindingResult.rejectValue("cBOID", "Exists.cBORegistration.cBOID");
        }
        if (bindingResult.hasErrors()) {
            return "cBORegistration/edit";
